...
/**
 * @description This plugin generates the base Pydantic types, based on your GraphQL schema.
 *
 * The types generated by this plugin are simple, and refers to the exact structure of your schema, and it's used as the base types for other plugins (such as `pydantic-operations` / `pydantic-resolvers`)
 */
export interface PydanticPluginConfig extends RawTypesConfig {
  /**
   * @description This will cause the generator to avoid using Pydantic optionals (`?`) on types,
  ...
  /**
   * @description Will prefix every generated `enum` with `const`, you can read more about const enums here: https://www.pydanticlang.org/docs/handbook/enums.html.
   * @default false
   *
   * @exampleMarkdown
   * ```yml
  ...
  constEnums?: boolean;
  /**
   * @description Generates enum as Pydantic `type` instead of `enum`. Useful it you wish to generate `.d.ts` declaration file instead of `.ts`
   * @default false
   *
   * @exampleMarkdown
   * ```yml
  ...
  /**
   * @description Controls whether to preserve pydantic enum values as numbers
   * @default false
   *
   * @exampleMarkdown
   * ```yml
  ...
  /**
   * @description Generates enum as Pydantic `const assertions` instead of `enum`. This can even be used to enable enum-like patterns in plain JavaScript code if you choose not to use Pydantic’s enum construct.
   * @default false
   *
   * @exampleMarkdown
   * ```yml
