...
export class PydanticOperationVariablesToObject extends OperationVariablesToObject {
  constructor(
  ...
  public wrapAstTypeWithModifiers(baseType: string, typeNode: TypeNode, applyCoercion = false): string {
    if (typeNode.kind === Kind.NON_NULL_TYPE) {
      ...
      return this.wrapMaybe(
        `${this._immutableTypes ? 'Tuple' : 'List'}[${innerType}${
          this._immutableTypes ? ', ...' : ''
        }]${listInputCoercionExtension}`
      );
    } else {
  ...
  protected wrapMaybe(type?: string) {
    const prefix = this._namespacedImportName ? `${this._namespacedImportName}.` : '';
    return `${prefix}${type ? `Optional[${type}]` : ''}`;
  }
